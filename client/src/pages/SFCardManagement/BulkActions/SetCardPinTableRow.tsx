...
const SetCardPinTableRow = (props: any) => {
  const {
    row,
    index,
    open,
  ...
  let newPINFieldName = `${row.fleetCardID}_N`;
  let confirmPINFieldName = `${row.fleetCardID}_C`;
      ...
      <TableRow className={classes.root}>
        ...
        <TableCell align="left">
          <OutlinedInput
            ...
            type="password"
            id={newPINFieldName}
            name={newPINFieldName}
            aria-label="New PIN"
            inputProps={{ maxLength: 4}}
            onChange={(event) => handleNewPINChange(event,row)}
            error={showPinError}
        ...
        <TableCell align="left">
          <OutlinedInput
              ...
              id={confirmPINFieldName}
              name={confirmPINFieldName}
              aria-label="Confirm PIN"
              inputProps={{ maxLength: 4}}
              onChange={(event) => handleConfirmPINChange(event,row)}
              error={showPinError}
              defaultValue={row?.confirmPin ? row.confirmPin : ''}
            />
        </TableCell>
      </TableRow>
...
export default SetCardPinTableRow;
