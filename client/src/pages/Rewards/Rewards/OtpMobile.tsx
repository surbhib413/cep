...
import { getResendOTPRewards } from "../../../lib/api/smartfleet/rewards/redeemBalance";

const OTP_MOBILE = '/otpMobile.svg';
const CancelIcon = "/Cancel_Icon.svg";
const BackIcon = "/Back_Icon.svg";

// START | ENTER OTP'S PARENT (INCOMPLETE)
...
//   //customerMobile={getMaskedMobileNumber(9090909090)}
//   otpData={otpData}
//   handleOtpVerify={handleOtpVerify}
// />
// END | ENTER OTP'S PARENT
...
export default function OtpMobile(props: any) {
  ...
  const resendOTP = async () => {
    ...
    let response: any = await getResendOTPRewards({});
    if (response?.status === "success") {
      setOtpSentMessage(response?.data?.message);
    } else {
      setOtpSentMessage("Please try again."); //set to api error message accordingly
    }
  ...
  const handleVerify = () => {
    const otpValue = Object.values(values).reduce(
      (otpString: any, value: any) => otpString + value
    );
  ...
  const refObjects = [textInput1, textInput2, textInput3, textInput4, textInput5, textInput6];
  ...
  const handleKeyUp = (event: React.KeyboardEvent<HTMLInputElement>, prop: keyof State) => {
    var key = event.key;
    let index: number = event.currentTarget.dataset.index ? parseInt(event.currentTarget.dataset.index) : 1;
  ...
  const validateEntry = (prop: keyof State) => (event: React.ChangeEvent<HTMLInputElement>): boolean => {
  ...
  return (
    <React.Fragment>
      {/* <Hidden smDown> */}
      <Dialog disableBackdropClick disableEscapeKeyDown fullScreen={fullScreen} open={props.open} onClose={handleClose} aria-labelledby="otp-dialog-title">
        <DialogTitle id="otp-dialog-title" className={classes.otpDialogTitle}>
          <div className={classes.otpDialogTitleSection} style={{ display: 'flex', flexDirection: 'row' }}>
            <div>
              <Hidden smUp>
                <img
                  src={BackIcon}
        ...
        <DialogContent>
          ...
          <div className={classes.otpContent1}>
            {props.otpData?.data?.message ? props.otpData?.data?.message : "An OTP (One Time Password) has been sent to registered number"}</div>
          <form className={classes.form} noValidate>
            ...
            <Typography className="text-center mt-2 mb-3">
              Didn’t receive OTP? <span className={classes.resend} onClick={() => resendOTP()}>Resend</span>
            </Typography>
          </form>
          ...
          <div style={{ margin: "1rem 0px", textAlign: "center" }}>
            <CustomButton
              variant="contained"
              disabled={!values.digit1 || !values.digit2 || !values.digit3 || !values.digit4 || !values.digit5 || !values.digit6}
              color="primary"
          ...
          <div style={{ margin: "1rem 0px" }}>
            <Warning open={errorMsg} onClose={() => setErrorMsg(false)}>Incorrect OTP entered. Please try again (2 attempts pending)</Warning>
            <Success open={resendMsg} onClose={() => setResendMsg(false)}>OTP resent</Success>
          </div>
        </DialogContent>
      </Dialog>
    </React.Fragment>
